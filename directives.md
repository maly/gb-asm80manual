...
| .include _filename:block_ | Include a part of a file, or better say: the block with given name from a file. The file is searched for the _.block name ... .endblock_ sequence. and the result is the same as if the block were copied in the current file instead of the INCLUDE line.<br>You can use the placeholder "this" as a file name. In that case parser will use the same file for looking, e.g. _.include this:common_ includes the _"common"_ block from the same file. |
